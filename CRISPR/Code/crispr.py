"""
Use genomic and coding sequences to create visual representation of gene, showing start and end positions with CRISPR sequences
CRISPR Sequences derived from : https://portals.broadinstitute.org/gppx/crispick/public

Functions:
- transcript_ensembl(ensembl, ID): Obtain information about the strand direction, exon intervals, transcript sequence,and coding sequence excluding 5' UTR
...
- get_gene_data(dir, infile, outfile): Takes the information extracted from transcript_ensembl to create a data file representing the extracted data in a .csv file format
    -Input: File directory(dir), File containing genes and their respective transcript IDs(infile) and the name of the .csv file created by the function(outfile)
    -Output: .csv file whose name is determined by the outfile parameter and stored in the given file directory
-extract_CRISPR_sequences(ensembl): Processes the CRISPR sequences obtained from CRIsPick and formats them into a useable format that will allow for them to be searched for in the genomic sequences
    - Input: EnsemblRelease version(ensembl), picking results file obtained from CRISPick.com(crispr_data), data file containing all the genes and their respective transcript ids(gene_transcripts)
    - Output: New data file containing formatted version of the gene_name followed by the CRISPR Sequences to be searched for(reverse complemented if found on the antisense strand) in a list format
-draw_CRISPR_site_for_ko(ensembl, geneID): Creates visual representation for models using a CRISPR knockout mechanism
    -Inputs: EnsemblRelease version(ensembl), gene ID of gene(geneID), data file created from get_gene_data(gene_data), picking result file from CRISPick(crispr_data), data file created from extract_CRISPR_sequences(crispr_seq_data)
    -Output: Visual representation of gene, showing exon positions as well as the positions of the gene's respective CRISPR knockout sequences denoted by C1-C5 followed by the number exon on which they lie
-draw_CRISPR_site_for_activated_deactivated(ensembl, geneID, mechanism): Creates visual representation for models using a CRISPR inactivation or activation mechanism
    -Inputs: EnsemblRelease version(ensembl), gene ID of gene(geneID), mechanism parameter which is either 'CRISPRi' if using an inactivation mechanism or 'CRISPRa' if using an activation mechanism, data file created from get_gene_data(gene_data), picking result file from CRISPick(crispr_data), data file created from extract_CRISPR_sequences(crispr_seq_data)
    -Output: Visual representation of gene, showing exon positions as well as the positions of the gene's respective CRISPR knockout sequences denoted by C1-C5 relative to the transcription start site , as well as the location of the transcription start site(TSS)

Both the drawing functions import the sub_seq function from NCBI_getGRCh38.py which uses Biopython's Entrez to search the NCBI database to obtain a gene's full genomic sequence, transcription start site, sub sequence determined by the minus and plus parameters, and the start and end positions of the gene
...
def get_gene_data(ensembl,dir, infile, outfile):
    ...
    for transcriptID in transcripts:
        ...
        print(header)
    print('done')
    save_dict = {'Header':headings, 'Gene Sequence':gene_seqs, 'Transcript Sequence':seqs, 'Exon Intervals':exons_s, 'Strand Direction':strands}
...
def draw_CRISPR_site_for_ko(ensembl, geneID):
    ...
    for index,sequence in enumerate(crispr_list_new):
        ...
        crispr_feature = GraphicFeature(start=seq_start, end=seq_end, strand=strand, color="#ff66ff", label='C'+str(index+1)+': ex'+str(exon_num), label_link_color='auto')
...
#    features.append(line_feature)
    ...
    gs = GridSpec(2, 1, height_ratios=[4,1])  # 3 rows: 2/3 for figure, 1/3 for table
    ax1 = fig.add_subplot(gs[0])
    ax1.set_title((gene_name + '(' + mechanism_pam + '), Chromosome '+str(chromosome_num)), x=0.5, y=0.5)
...
def draw_CRISPR_site_for_activated_deactivated(ensembl, geneID, mechanism):
    ...
    for x in range(len(sgrna_seqs)):
        data = ['C'+str(x+1) + ' + '+ pam_sequences[x], strands[x], cut_sites[x], eff_scores[x]]
    ...
    ax1.set_title((gene_name + '(' + mechanism_pam + '), '+'Chromosome '+ str(chromosome_num)), x=0.5, y=0.5)
    #    fig, (ax1, ax2) = plt.subplots(nrows=2,ncols=1, figsize=(8,4))
    record.plot(ax=ax1, figure_width=6)
    if mechanism == 'CRISPRa':
...
if __name__ == "__main__":
    ...
    if mode == 'get_gene_data':
        ...
        data = pd.read_excel(dir + infile)
        transcripts = data['ENSEMBL']
        for num in range(248,len(transcripts)):
            print(num)
        ...
        data = pd.read_excel(dir + infile)
        transcripts = data['ENSEMBL']
        for num in range(349 ,len(transcripts)):
#        num = random.randint(0, len(transcripts) - 1)
            print(num)
            transcriptID = transcripts[num]
            transcript = ensembl.transcript_by_id(transcriptID)
            geneID = transcript.gene_id
            draw_CRISPR_site_for_activated_deactivated(ensembl, geneID, 'CRISPRi')
